...
class ParserTest {
    ...
    fun testCommands() {
        ...
        assertEquals(
            Pair("", listOf(listOf("wc", "-l", "--files0-from=file name.txt"))),
            commands.parse("wc -l --files0-from='file name.txt'")
        )
        assertEquals(
            Pair("", listOf(listOf("cat", "/etc/hosts"), listOf("wc", "-l"))),
            commands.parse("cat /etc/hosts | wc -l")
        )
